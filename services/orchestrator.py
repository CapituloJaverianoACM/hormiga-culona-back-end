...
class AgentOrchestratorService:
    """Orquesta agentes, audio y acceso a datos."""
    ...
    def __init__(self):
        ...
        self.audio_transcription_client = AzureTranscriptionClient()
        self.audio_synthesis_client = AzureSpeechSynthesisClient()
        ...
        asyncio.run(self.result_agent.create_agent(self.open_ai_client, database_schema))
    ...
    async def process_text_request(
        ...
        result_data = await self._build_query_data_async(content, preview_limit)
        ...
        response_data = {
            "agent_reply": result_data["explanation"],
            "summary": result_data["summary"],
            "explanation": result_data["explanation"],
            "voice_reply": result_data["voice_reply"],
            "sql": result_data["sql"],
            "columns": result_data["columns"],
            "preview_rows": result_data["preview_rows"],
    ...
    async def _build_query_data_async(self, content: str, preview_limit: int = 5) -> dict[str, Any]:
    ...
    def transcribe_audio(self, audio_bytes: bytes, filename: str = "audio.wav", content_type: str = "audio/wav") -> str:
        return self.audio_transcription_client.transcribe_audio(audio_bytes, filename, content_type)

    def synthesize_audio(self, text: str) -> bytes:
        return self.audio_synthesis_client.synthesize_speech(text)
